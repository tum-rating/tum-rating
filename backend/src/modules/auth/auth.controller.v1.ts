...
export class AuthControllerV1 {
  ...
  @Post('/signup')
  public async signUp(
    @Body(new JoiObjectSchemaPipe(SignUpRequestSchema)) body: SignUpRequestDto,
  ) {
  ...
  public async signin(
    @Body(new JoiObjectSchemaPipe(SignInRequestSchema)) body: SignInRequestDto,
    ...
    if (!databaseUser) {
      this._logger.warn(
        'Sign in request fail, user does not exsit for %s',
        body.email,
      );
      const activationToken = await this._jwtService.signJWTActivate(
        databaseUser.id,
