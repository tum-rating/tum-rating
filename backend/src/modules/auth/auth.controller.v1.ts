...
import {
    ActivateUserEmailRequestDto,
    ActivateUserEmailRequestSchema,
} from './dto/ActivateUserEmail.dto';
import {
...
export class AuthControllerV1 {
    ...
    @Post('/signup')
    public async signUp(
        @Body(new JoiObjectSchemaPipe(SignUpRequestSchema))
        body: SignUpRequestDto,
    ) {
        this._logger.info(
            'Signup request received with user email %o',
            body.email,
        );

        const acceptedEmailDomains = this._configService.getOrThrow<string[]>(
            'signup.acceptedEmailDomains',
        );
        if (!acceptedEmailDomains.includes(body.email.split('@')[1]))
            throw new BadRequestException(
                `Email domain must be of ${acceptedEmailDomains}`,
            );

        const passwordSalt = await this._authenticationService.getSalt();

        const passwordHash = await this._authenticationService.getHash(
            body.password,
            passwordSalt,
        );
        ...
        try {
            ...
            this._logger.debug(
                'Preparing activation token for email: %s',
                body.email,
            );

            const activationToken = await this._jwtService.signJWTActivate(
                createdUser.id,
            );
            ...
            if (error.code == ERROR_MONGO_DUPLICATE_CODE) {
                ...
                this._logger.info(
                    'Signup duplicate "%s" already exists %s, %s',
                    errorMessage,
                    body.username,
                    body.email,
                );
    ...
    public async signin(
        ...
        this._logger.info(
            'Signin request received for user with email %s',
            body.email,
        );
        ...
        if (!databaseUser) {
            this._logger.warn(
                'Sign in request fail, user email does not exsit for %s',
                body.email,
            );
        ...
        if (!databaseUser.isEmailActivated) {
            this._logger.warn(
                'Sign in request fail, user email is not activated for %s',
                body.email,
            );
            const activationToken = await this._jwtService.signJWTActivate(
                databaseUser.id,
        ...
        if (passwordHash !== databaseUser.passwordHash) {
            this._logger.warn(
                'Sign in request fail, password does not match for %s',
                body.email,
            );
            throw new UnauthorizedException();
        }

        const token = await this._jwtService.signJWTAccess(
            databaseUser.id,
            databaseUser.role,
        );

        this._logger.info(
            'Signin request completed user with email %s',
            body.email,
        );
    ...
    public async activateEmail(
        @Body(new JoiObjectSchemaPipe(ActivateUserEmailRequestSchema))
        body: ActivateUserEmailRequestDto,
    ) {
        const { isValid, payload } = await this._jwtService.verifyJWTActivate(
            body.token,
        );
        ...
        this._logger.info(
            'Activate email request received for user %o',
            payload.sub,
        );
        ...
        this._logger.info(
            'Activate email request completed for user %s',
            payload.sub,
        );
    }
    ...
    public async passwordRecovery(
        @Body(new JoiObjectSchemaPipe(PasswordRecoveryRequestSchema))
        body: PasswordRecoveryRequestDto,
    ) {
        if (body.email) {
            this._logger.info(
                'Password recovery request for email %s',
                body.email,
            );
            ...
            if (!user) {
                this._logger.warn(
                    'Email reqested for recovery is not in codebase %s',
                    body.email,
                );
                return;
            }

            const recoveryTokoen = await this._jwtService.signJWTRecovery(
                user.id,
            );

            await this._mailerService.sendPasswordRecoveryEmail(
                [{ email: user.email, name: user.username }],
                recoveryTokoen,
            );
            this._logger.info(
                'Succesfully send recovery email to %s',
                user.email,
            );
            return;
        }
        ...
        if (body.token && body.password) {
            const { isValid, payload } =
                await this._jwtService.verifyJWTRecovery(body.token);

            if (!isValid) throw new UnauthorizedException();

            this._logger.info(
                'Password recovery request for user %s',
                payload.sub,
            );
            ...
            this._logger.info(
                'Succesfully updated password for user %s',
                payload.sub,
            );
            return;
        }
        ...
        this._logger.error(
            'Password recover invalid body after pipe validation body: %o',
            body,
        );
