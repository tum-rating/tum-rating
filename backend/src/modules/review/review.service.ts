...
import {
    CreateReviewUserType,
    PatchReviewUserType,
    ReviewUserRepository,
} from 'src/database/repositories/reviewUser.repository';
...
export class ReviewService {
    ...
    public async getReviewsOverviewPaginated(
        pageNumber: number,
        pageSize: number,
        search?: string,
    ) {
        const results = await this._reviewRepository.getReviewsByQuery(
            pageNumber,
            pageSize,
            search,
        );
    ...
    public async getReviewByIdWihtPopulatedReviewsUser(id: string) {
        const review =
            await this._reviewRepository.findOneByIdWithPopulatedReviews(id);
    ...
    public async getReviewUser(reviewId: string, userId: string) {
        const reviewUser =
            await this._reviewUserRepository.getOneByReviewIdAndUserId(
                reviewId,
                userId,
            );
        ...
        if (reviewUser === null)
            throw new NotFoundError(
                `review ${reviewId} user ${userId} not found`,
            );

        return reviewUser;
    }

    public async addReviewUser(reviewUser: CreateReviewUserType) {
        const review = await this.getReviewById(
            reviewUser.reviewId as unknown as string,
        );

        if (!review.offeredInSemesters.includes(reviewUser.semester))
            throw new UserReviewSemesterMismatch(reviewUser.semester);

        const createdReviewUser =
            await this._reviewUserRepository.create(reviewUser);

        await this._reviewRepository.addReviewUser(
            review.id,
            createdReviewUser.id,
        );
    ...
    public async updateReviewStats(reviewId: string) {
        const stats =
            await this._reviewUserRepository.getStatsByReviewId(reviewId);
    ...
    public async patchReviewUser(
        reviewId: string,
        userId: string,
        patchUserReview: PatchReviewUserType,
    ) {
        const review = await this._reviewRepository.findOneById(reviewId);

        if (review === null) throw new NotFoundError('review not found');

        if (
            patchUserReview.semester &&
            !review.offeredInSemesters.includes(patchUserReview.semester)
        )
            throw new UserReviewSemesterMismatch(patchUserReview.semester);

        const updateResult =
            await this._reviewUserRepository.updateOneByUserIdAndReviewId(
                userId,
                reviewId,
                patchUserReview,
            );
