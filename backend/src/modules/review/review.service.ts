...
export class ReviewService {
    ...
    public async getReviewUser(reviewId: string, userId: string) {
        ...
        if (reviewUser === null) throw new NotFoundError(`review ${reviewId} user ${userId} not found`);
    ...
    public async addReviewUser(reviewUser: CreateReviewUserType) {
        ...
        if (!review.offeredInSemesters.includes(reviewUser.semester)) throw new UserReviewSemesterMismatch(reviewUser.semester);
    ...
    public async deleteReviewUser(userId: string, reviewId: string) {
        return this._reviewUserRepository.deleteOneByUserIdAndReviewId(userId, reviewId);
    }
