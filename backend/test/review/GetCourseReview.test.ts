...
describe('Get Course Review', () => {
        ...
        return supertest(reviewUrl)
            ...
            .expect((response: supertest.Response) => {
                ...
                expect(response.body.professor).toEqual(
                    createdReview.professor,
                );
                expect(response.body.courseNumber).toEqual(
                    createdReview.courseNumber,
                );
            ...
            .expect((response: supertest.Response) => {
                ...
                expect(
                    response.body.reviews.find(
                        (review) => review._id === createdReview.id,
                    ),
                ).toBeDefined();
            });
    });
    ...
    it('should search by parital course title', async () => {
        ...
        const createdReview1 = await createCourseReviewMockRequest(
            signInResponse.token,
            {
                course: title1,
            },
        ...
        const createdReview2 = await createCourseReviewMockRequest(
            signInResponse.token,
            {
                course: title2,
            },
        ...
        const createdReview3 = await createCourseReviewMockRequest(
            signInResponse.token,
            {
                course: title3,
            },
        ...
        return supertest(reviewUrl)
            ...
            .expect((response: supertest.Response) => {
                ...
                expect(
                    response.body.reviews.find(
                        (review) => review._id === createdReview1.id,
                    ),
                ).toBeDefined();
                expect(
                    response.body.reviews.find(
                        (review) => review._id === createdReview2.id,
                    ),
                ).toBeDefined();
                expect(
                    response.body.reviews.find(
                        (review) => review._id === createdReview3.id,
                    ),
                ).toBeDefined();
            });
    });
    ...
    it('should return paginated reviews', async () => {
        ...
        await Promise.all(
            titles.map((title) => deleteReviewsWithCourseTitle(title)),
        );
        ...
        const paginationResponse = await supertest(reviewUrl)
            ...
            .expect((response: supertest.Response) => {
                ...
                for (let i = 0; i < 5; i++) {
                    expect(
                        response.body.reviews.find(
                            (review) => review._id === createdReviews[i].id,
                        ),
                    ).toBeDefined();
                }
            });
        ...
        const paginationResponse2 = await supertest(reviewUrl)
            .get(
                `?search=${commonTitlePart}&page-size=5&page-number=` +
                    paginationResponse.body.nextPageNumber,
            )
            .expect(200)
            .expect((response: supertest.Response) => {
                expect(response.body).toHaveProperty('reviews');
        ...
        return supertest(reviewUrl)
            .get(
                `?search=${commonTitlePart}&page-size=5&page-number=` +
                    paginationResponse2.body.nextPageNumber,
            )
            .expect(200)
            .expect((response: supertest.Response) => {
                expect(response.body).toHaveProperty('reviews');
