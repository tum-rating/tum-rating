    ...
    if (!user) {
        return (
            <Flex direction="column" gap="xs" h="100%" justify="center" align="center" py="md">
                ...
                <Text fw="600">Only registered users can add reviews.</Text>
                <Button onClick={() => navigate(Paths.signIn)} mt="md" fullWidth>Sign In</Button>
                <Divider w="100%" variant="dotted" my="xs" size="md" label="or" labelPosition="center"/>
                <Button onClick={() => navigate(Paths.signUp)} fullWidth variant="outline">Sign Up</Button>
            </Flex>
        )
    }
        ...
        <Container px={0} pos="relative" h="100%">
            ...
            >
                <Flex direction="column" gap="xs" h="100%">
                    <Textarea autosize minRows={6} maxRows={6} placeholder="Your comment" label="Your comment" h="auto"
                              value={form.values.comment} {...form.getInputProps('comment')}
                              onChange={(event) => form.setFieldValue('comment', event.currentTarget.value)}/>
                    <Select {...form.getInputProps('semester')} label="Semester" placeholder="Semester"
