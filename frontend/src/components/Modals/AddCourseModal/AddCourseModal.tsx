    ...
    return (
        <Container p={0} data-testid="modal-content" h={visualViewport}>
            ...
            <form className="modal-form" data-testid="form" onSubmit={form.onSubmit((e) => handleSubmit(e))}>
                <Box p="sm">
                    <Stack>
                        <Textarea data-testid="textarea" label="Course URL" required
                                  placeholder={`Example: ${example_course}`}
                                  description="Provide a valid course URL from TUM Campus Portal"
                                  value={form.values.url}
                                  onChange={(event) => form.setFieldValue('url', event.currentTarget.value)}
                                  error={form.errors.url} radius="md" minRows={5} maxRows={10} autosize/>
                        {apiError && error && (
                        ...
                        <Button data-testid="submit-button" loading={addReviewLoading} mt="xs" type="submit"
                        ...
                        <Alert variant="light" color="green" title="How to add a course" icon={<IconInfoCircle/>}>
                            <Text size="sm">
                                ...
                                . Copy the link and paste it into our form. If everything is ok, the course will appear
                                within 24 hours.
