...
export const HowEasyRating = (props: HowEasyRatingProps) => {
    ...
    return (
        <Stack align="center" spacing="0">
            <Text weight={500}>How Easy</Text>
            <Flex gap={10} align="center">
                <Text fz={36} weight={500}>{score}</Text>
                <Text>/ 5</Text>
            </Flex>
            <Rating
                mb={10}
                ...
                fractions={2}
                readOnly={readonly}
                onChange={(value) => readonly ? null : setScore(value)} // Update the score only if not readonly
            />
            <Badge color={message === 'Very Difficult' ? 'red' : (message === 'Moderate' ? 'yellow' : 'green')}>
                {message}
            </Badge>
        </Stack>
    );
