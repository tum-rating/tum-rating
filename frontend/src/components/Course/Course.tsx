...
const Course = () => {
    ...
    useEffect(() => {
        if (data?.examStats) {
            const sortedExamStats = Object.entries(data.examStats)
                .flatMap(([semester, exams]) =>
                    Object.entries(exams).map(([examType, examData]) => ({
                        semester,
                        examType,
                        ...examData
                    }))
            ...
            console.log(sortedExamStats)
            setExamResults(sortedExamStats as FormatedExamStats[]);
        }
    }, [data]);
        ...
        <>
            ...
            >
                {isError ? (
                    ...
                    <>
                        <CourseControls data={data} isLoading={isLoading} user={user}
                                        userReview={userReview}></CourseControls>
                        <Box className={classes.courseContent} data-testid="course-content">
                            <Flex className={clsx(classes.courseBanner, 'children-animation')}>
                                <Box className={classes.image}>
                                    <Skeleton radius="lg" mah={90} w={280} h={100} loading={isLoading}
                                              component={<Image radius="lg" h={100} mah={90} w={280} fit="contain"
                                                                fallbackSrc={tumLogo}/>}/>
                                </Box>
                                    ...
                                    <Skeleton
                                        ...
                                        component={
                                            <CourseTitleAnchor courseId={data?.courseId}>
                                                <Text data-testid="course-name" style={{wordBreak: 'break-word'}}
                                                      fz={24} fw="700" lineClamp={5}>
                                                    {data?.name}{' '}
                                                </Text>
                                            </CourseTitleAnchor>
                                        }
                            ...
                            <Flex mt="xl" direction="column" className="children-animation"
                                ...
                                <Flex gap="lg" wrap="wrap">
                                    ...
                                    <HowInterestingRating isLoading={isLoading}
                                                          score={data?.howInterestingRatingAverage}/>
                            ...
                            <Flex style={{flexGrow: 1, background: 'var(--mantine-color-body)'}} pt="xl"
                                  direction="column" className="children-animation">
                                <Flex justify="space-between" align="center" mb="lg">
                                    ...
                                    <Box hiddenFrom="sm">
                                        {user ? (
                                            userReview ? (
                                                <Button data-testid="edit-review" loading={isLoading} size="sm"
                                                        variant="gradient"
                                                ...
                                                <Button data-testid="add-review" loading={isLoading} size="sm"
                                                        variant="gradient"
                                            ...
                                            <>
                                                <Button loading={isLoading} size="sm" variant="primary-gradient"
