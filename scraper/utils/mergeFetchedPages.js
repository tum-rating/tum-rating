...
const mergeFetchedPages = async (filesToMerge) => {
    ...
    allData.forEach((course, index) => {
        ...
        const mainLecturer = course.mainLecturers.length > 0 ? course.mainLecturers[0].name : "";
        const key = `${courseTitle}-${mainLecturer}`;
        if (mergedCourses[key]) {
            if (!mergedCourses[key].semester.includes(course.semester[0])) mergedCourses[key].semester.push(course.semester[0]);
        } else {
            mergedCourses[key] = {...course};
        }
    });
