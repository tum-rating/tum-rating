...
const queryParams = '$filter=courseNormKey-eq=LVEAB;orgId-eq=1;termId-eq=198&$orderBy=title=ascnf';
const pageSize = 20;
...
fetchAllPages()
    .then(courses => {
        ...
        const summaries = courses.map(course => {
            ...
            const courseTitleTranslations = course.content.cpCourseDto.courseTitle.translations.translation.reduce((acc, translation) => {
            ...
            course.content.cpCourseDto.lectureships.forEach((lecturer, index) => {
                ...
                const lecturerInfo = {
                    name: lecturerName,
                    businessCardLink: lecturer.identityLibDto.businessCardLink ? lecturer.identityLibDto.businessCardLink.href : null
                };
