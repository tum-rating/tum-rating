  ...
  if (response.value.length) {
    for (let semesterId of response.value) {
      try {
        ...
        const filename = `./fetched/${semestersData[semesterId].replaceAll(' ', '-')}-${semesterId}-${new Date()
            .toLocaleString()
            .replaceAll(', ', '-')}.json`;
...
const mergeFiles = async () => {
  ...
  try {
    await generateHTMLForJSONCompare(selectedFiles);
