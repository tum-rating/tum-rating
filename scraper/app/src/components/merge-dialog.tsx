...
const MergeDialog = (props: PropsWithChildren) => {
    const {files, keySimilarityMerging} = useContext(AppDataContext)!;
        ...
        <Dialog open={open} onOpenChange={setOpen}>
            ...
            <DialogContent className={'max-w-[700px]'}>
                <DialogHeader>
                    ...
                    <DialogDescription>
                        Here you can merge courses from different sources with selected method
                    </DialogDescription>
                </DialogHeader>
                        ...
                        {method && (
                            <>
                                ...
                                <div className="flex gap-2 flex-wrap">
                                    {files.map((file) => {
                                        const isDisabled = !METHODS_SOURCE_MAP.find(m => m.type === method)?.fileType.some(ft => file.name.includes(ft));
                                        return (
                                            <div
                                                key={file.name}
                                                ...
                                                onClick={() => !isDisabled && toggleSelectItem(file.name)}
                                            >
                                                {file.name}
                                            </div>
                                        );
