...
function keySimilarityCore(
    ...
    const codePattern =
        /\b([A-Z]{2,}[0-9]+(?:_[0-9]+)?(?:\s+[A-Z]+)?(?:\s+[A-Z]{2})?)\b/g;
    ...
    function redundantWordsExtractor(title: string): {
        cleanedTitle: string;
        codes: Set<string>;
    } {
        ...
        const redundantWordsPattern = new RegExp(
            `\\b(${redundantWords.join("|")})\\b`,
            "gi",
        );
        const delimitersPattern = /[()\[\],-]/g;
        const cleanedTitle = title
            .replace(redundantWordsPattern, "")
            .replace(codePattern, "")
    ...
    function checkIfCoursesHaveAtLeastOneCommonWord(
        course1: string,
        course2: string,
    ): boolean {
    ...
    for (let index = 0; index < arr.length; index++) {
        ...
        const {cleanedTitle: itemTitle, codes: currentCodes} =
            redundantWordsExtractor(item[key]);
        ...
        for (let i = index + 1; i < arr.length; i++) {
            ...
            const record = arr[i];
            const {cleanedTitle: recordTitle, codes: recordCodes} =
...
const summarizeFetchedData = async (
    courses: Course[],
    ...
    return courses.map((course) => {
        ...
        const englishTitle = course.courseTitle.translations.translation.find(
            (el) => el.lang === "en",
        )?.value;
        ...
        course.lectureships.forEach((lecturer, index) => {
            ...
            const lecturerInfo = {
                name: lecturerName,
                businessCardLink:
                    lecturer.identityLibDto.businessCardLink?.href || null,
            };
            if (
...
const parseCourse = (course: any): ParsedCourse => {
    ...
    if (course.mainLecturers.length > 1)
        parsedCourseForApi.otherLecturers.push(
            ...course.mainLecturers.slice(1).map((lecturer) => lecturer.name),
        );

    if (course.otherLecturers)
        parsedCourseForApi.otherLecturers.push(
            ...course.otherLecturers.map((lecturer) => lecturer.name),
        );
