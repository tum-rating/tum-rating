...
const getCheckingPrompt = () => `
You will receive sets of university course names that have actually been merged. The task is to check and confirm that the merged courses are well merged and that there is no bad merge. All checking should be based on AI MERGING RULES - this is very important, all evaluation if courses are well merged should be decided by these rules. The input structure is a JSON array of arrays, where each inner array represents a separate case to analyze:
[
  ["nominal_name1", "possible_match1", "possible_match2"],
  ["nominal_name2", "possible_match3", "possible_match4"]
]
First element in each inner array is always a nominal name, and the rest are merged courses.
...
export const checkCorrectnessOfMergedCourses = async (
    courses: string[] | string[][],
    wss?: WebSocketServer,
    fileName: string
): Promise<AiResponse | AiResponse[]> => {
